...
def test_product_recommendation_form(client):
    resp = client.post('/api/v1/recommendations', data={'query': 'phone', 'top_k': 2})
    assert resp.status_code == 200
    data = resp.get_json()
    assert data['status'] == 'success'
    assert 'products' in data
...
def test_ocr_query_no_file(client):
    resp = client.post('/api/v1/ocr-query')
    assert resp.status_code == 400
    data = resp.get_json()
    assert data['status'] == 'error'
...
def test_ocr_query_with_file(client):
    ...
    resp = client.post('/api/v1/ocr-query', content_type='multipart/form-data', data=data)
